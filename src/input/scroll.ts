...
function get_arrow_key_value(arrow: key | null) {
    if (!arrow) {
        return 0;
    }
...
function get_normalized_arrow_key_value(arrow: key | null): number {
    let value: number = get_arrow_key_value(arrow);
    if (value == SCROLL_KEY_PRESSED) {
        return 1.0;
    } else {
        return Math.min((arrow ? arrow.value : 0) / SCROLL_KEY_MAX_VALUE, 1.0);
    }
}
function is_arrow_active(arrow: key | null) {
    return arrow ? arrow.value != 0 : 0;
}
function restart_active_arrow(arrow: key | null, exception: key | null) {
    if (!arrow) {
        return;
    }
    if (arrow == exception) {
...
export function scroll_drag_start(is_touch: number) {
    ...
    data.drag.delta.y = 0;
    if (!is_touch) {
        const reset_x = new Ref(0);
        const reset_y = new Ref(0);
        system_mouse_get_relative_state(reset_x, reset_y);
    }
    clear_scroll_speed();
}
function set_scroll_speed_from_drag() {
    ...
    if (!data.drag.is_touch) {
        const relative_x = new Ref(0);
        const relative_y = new Ref(0);
...
function set_arrow_input(arrow: key | null, opposite_arrow: key | null, modifier: number): number {
    if (get_arrow_key_value(arrow) && (!opposite_arrow || !is_arrow_active(opposite_arrow))) {
        if (arrow && arrow.state == KEY_STATE_AXIS) {
...
function set_scroll_speed_from_input(m: mouse, type: scroll_type) {
    ...
    if (!config_get(CONFIG_UI_SMOOTH_SCROLLING) && !data.limits.active) {
        ...
        if (type == SCROLL_TYPE_CITY) {
            const camera_x = new Ref(0);
            const camera_y = new Ref(0);
