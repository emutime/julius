...
import { building_market_get_max_food_stock } from 'building/market';
import { house_level } from 'building/type';
import { low_mood_cause } from 'city/constants';
import { city_culture_average_education, city_culture_average_entertainment, city_culture_average_health } from 'city/culture';
import { city_figures_enemies } from 'city/figures';
import { city_god_happiness, MAX_GODS } from 'city/gods';
import { city_labor_unemployment_percentage, city_labor_workers_needed } from 'city/labor';
import { city_population } from 'city/population';
import { city_resource_food_supply_months } from 'city/resource';
...
import { figure, figure_get, figure_is_enemy } from 'figure/figure';
import { trader_has_traded } from 'figure/trader';
import { figure_type } from 'figure/type';
import { figure_trade_caravan_can_buy, figure_trade_caravan_can_sell, figure_trade_ship_is_trading, trade_ship } from 'figuretype/trader';
...
import HOUSE_SMALL_HOVEL = house_level.HOUSE_SMALL_HOVEL;
import HOUSE_LARGE_CASA = house_level.HOUSE_LARGE_CASA;
...
export const enum god_state {
    GOD_STATE_NONE = 0,
    GOD_STATE_VERY_ANGRY = 1,
    GOD_STATE_ANGRY = 2,
}
import GOD_STATE_NONE = god_state.GOD_STATE_NONE;
import GOD_STATE_VERY_ANGRY = god_state.GOD_STATE_VERY_ANGRY;
import GOD_STATE_ANGRY = god_state.GOD_STATE_ANGRY;

let FIGURE_SOUNDS: string[][] = [
    [ // 0
...
let FIGURE_TYPE_TO_SOUND_TYPE: number[] = [
...
function play_sound_file(sound_id: number, phrase_id: number) {
    if (sound_id >= 0 && phrase_id >= 0) {
        let path: string = "wavs/" + FIGURE_SOUNDS[sound_id][phrase_id];
        sound_speech_play_file(path);
    }
}
export function figure_phrase_play(f: figure) {
    if (f.id <= 0) {
        return 0;
    }
    let sound_id: number = FIGURE_TYPE_TO_SOUND_TYPE[f.type];
    play_sound_file(sound_id, f.phrase_id);
    return sound_id;
}
function lion_tamer_phrase(f: figure) {
...
function gladiator_phrase(f: figure) {
    return f.action_state == FIGURE_ACTION_150_ATTACK ? 7 : 0;
}
function tax_collector_phrase(f: figure) {
...
function market_trader_phrase(f: figure) {
...
function market_buyer_phrase(f: figure) {
...
function cart_pusher_phrase(f: figure) {
...
function warehouseman_phrase(f: figure) {
...
function prefect_phrase(f: figure) {
...
function engineer_phrase(f: figure) {
...
function citizen_phrase(f: figure) {
    if (++f.phrase_sequence_exact >= 3) {
        f.phrase_sequence_exact = 0;
    }
    return 7 + f.phrase_sequence_exact;
}
function house_seeker_phrase(f: figure) {
    if (++f.phrase_sequence_exact >= 2) {
        f.phrase_sequence_exact = 0;
    }
    return 7 + f.phrase_sequence_exact;
}
function emigrant_phrase() {
...
function tower_sentry_phrase(f: figure) {
    if (++f.phrase_sequence_exact >= 2) {
        f.phrase_sequence_exact = 0;
    }
    let enemies: number = city_figures_enemies();
    if (!enemies) {
        return 7 + f.phrase_sequence_exact;
    } else if (enemies <= 10) {
        return 9;
...
function docker_phrase(f: figure) {
...
function trade_caravan_phrase(f: figure) {
    if (++f.phrase_sequence_exact >= 2) {
        f.phrase_sequence_exact = 0;
    }
...
function trade_ship_phrase(f: figure) {
    if (f.action_state == FIGURE_ACTION_115_TRADE_SHIP_LEAVING) {
        ...
        let state: number = figure_trade_ship_is_trading(f);
...
function phrase_based_on_figure_state(f: figure) {
    switch (f.type) {
...
function city_god_state() {
    let least_god_happiness: number = 100;
    for (let i: number = 0; i < MAX_GODS; i++) {
        let happiness: number = city_god_happiness(i);
...
function phrase_based_on_city_state(f: figure) {
    f.phrase_sequence_city = 0;
    let god_state: number = city_god_state();
    let unemployment_pct: number = city_labor_unemployment_percentage();
...
export function figure_phrase_determine(f: figure) {
    if (f.id <= 0) {
        return;
    }
    f.phrase_id = 0;
    ...
    let phrase_id: number = phrase_based_on_figure_state(f);
    if (phrase_id) {
