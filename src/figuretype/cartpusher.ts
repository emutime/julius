...
import { building_barracks_add_weapon, building_get_barracks_for_weapon } from 'building/barracks';
import { building, building_get } from 'building/building';
import { building_getting_granary_for_storing, building_granary_add_resource, building_granary_for_getting, building_granary_for_storing, building_granary_remove_for_getting_deliveryman, building_granary_remove_resource } from 'building/granary';
import { building_get_workshop_for_raw_material, building_get_workshop_for_raw_material_with_room, building_workshop_add_raw_material } from 'building/industry';
import { building_state, building_type } from 'building/type';
import { building_warehouse_add_resource, building_warehouse_for_getting, building_warehouse_for_storing, building_warehouse_remove_resource } from 'building/warehouse';
...
import { map_routing_citizen_is_passable, map_routing_citizen_is_passable_terrain } from 'map/routing_terrain';
...
let CART_OFFSET_MULTIPLE_LOADS_FOOD: number[] = [0, 0, 8, 16, 0, 0, 24, 0, 0, 0, 0, 0, 0, 0, 0, 0];
let CART_OFFSET_MULTIPLE_LOADS_NON_FOOD: number[] = [0, 0, 0, 0, 0, 8, 0, 16, 24, 32, 40, 48, 56, 64, 72, 80];
let CART_OFFSET_8_LOADS_FOOD: number[] = [0, 40, 48, 56, 0, 0, 64, 0, 0, 0, 0, 0, 0, 0, 0, 0];
...
function determine_cartpusher_destination(f: figure, b: building, road_network_id: number) {
    let dst: map_point;
    let understaffed_storages: Ref<number> = new Ref(0);
    ...
    f.wait_ticks = 0;
    f.min_max_seen = understaffed_storages.v ? 2 : 1;
}
...
function determine_warehouseman_destination(f: figure, road_network_id: number) {
    ...
    dst_building_id = building_granary_for_storing(f.x, f.y, f.resource_id,
        warehouse.distance_from_entry, road_network_id, 0, null, dst);
    if (dst_building_id) {
        set_destination(f, FIGURE_ACTION_51_WAREHOUSEMAN_DELIVERING_RESOURCE, dst_building_id, dst.x, dst.y);
        remove_resource_from_warehouse(f);
        return;
    ...
    dst_building_id = building_warehouse_for_storing(f.building_id, f.x, f.y, f.resource_id,
        warehouse.distance_from_entry, road_network_id, null, dst);
    if (dst_building_id) {
...
export function figure_warehouseman_action(f: figure) {
    ...
    switch (f.action_state) {
        ...
        case FIGURE_ACTION_55_WAREHOUSEMAN_AT_GRANARY:
            ...
            if (f.wait_ticks > 4) {
                let resource: number;
                let resourceRef = new Ref(0);
                f.loads_sold_or_carrying = building_granary_remove_for_getting_deliveryman(
                    building_get(f.destination_building_id), building_get(f.building_id), resourceRef);
                f.resource_id = resourceRef.v;
