...
import { game_file_load_saved_game } from 'game/file';
import { game_file_write_saved_game } from 'game/file';
import { game_file_delete_saved_game } from 'game/file';
import { game_file_editor_load_scenario } from 'game/file_editor';
import { game_file_editor_write_scenario } from 'game/file_editor';
...
import { input_box_refresh_text } from 'widget/input_box';
...
let scrollbar: scrollbar_type = { 464, 120, 206, 320, NUM_FILES_IN_VIEW, on_scroll, 1};
export class file_type_data {
    public extension: char[] = new Array(4).fill(null);
    public last_loaded_file: char[] = new Array(FILE_NAME_MAX).fill(null);
...
export class unnamed65_8 {
    ...
    public selected_file: char[] = new Array(FILE_NAME_MAX).fill(null);
    public constructor(...args: any[]) {
...
let file_name_input: input_box = { 144, 80, 20, 2, FONT_NORMAL_WHITE, 0, data.typed_name, FILE_NAME_MAX };
let saved_game_data: file_type_data = { "sav"};
let scenario_data: file_type_data = { "map"};
function find_first_file_with_prefix(prefix: char) {
    let len: number = (int) strlen(prefix);
    if (len == 0) {
        return -1;
    }
    let left: number = 0;
...
function button_ok_cancel(is_ok: number, param2: number) {
    ...
    let filename: char = get_chosen_filename();
