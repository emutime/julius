...
;
export function building_animation_offset(b: building, image_id: number, grid_offset: number) {
    ...
    let new_sprite: number = 0;
    let is_reverse: number = 0;
    if (b.type == BUILDING_WINE_WORKSHOP) {
        let pct_done: number = calc_percentage(b.data.industry.progress, 400);
        if (pct_done <= 0) {
            new_sprite = 0;
        } else if (pct_done < 4) {
            new_sprite = 1;
        ...
        let current_sprite: number = map_sprite_animation_at(grid_offset) & 0x7f;
        if (is_reverse) {
