...
import { mouse, scroll_state } from 'input/mouse';
import { touch, touch_get_earliest } from 'input/touch';
...
let image_button_scroll_up: image_button = new image_button(
    0, 0, SCROLL_BUTTON_WIDTH, SCROLL_BUTTON_HEIGHT, ib.IB_SCROLL,
    group_terrain.GROUP_OK_CANCEL_SCROLL_BUTTONS, 8, text_scroll, button_none, 0, 1, 1
);
let image_button_scroll_down: image_button = new image_button(
    0, 0, SCROLL_BUTTON_WIDTH, SCROLL_BUTTON_HEIGHT, ib.IB_SCROLL,
    group_terrain.GROUP_OK_CANCEL_SCROLL_BUTTONS, 12, text_scroll, button_none, 1, 1, 1
);
...
export function scrollbar_init(scrollbar: scrollbar_type, scroll_position: number, total_elements: number) {
    ...
    scrollbar.touch_drag_state = touch_drag.TOUCH_DRAG_NONE;
}
...
export function scrollbar_draw(scrollbar: scrollbar_type) {
    if (scrollbar.max_scroll_position > 0 || scrollbar.always_visible) {
        ...
        image_draw(image_group(group_terrain.GROUP_PANEL_BUTTON) + 39,
...
function handle_touch(scrollbar: scrollbar_type, t: touch) {
    ...
    let active = scrollbar.touch_drag_state == touch_drag.TOUCH_DRAG_IN_PROGRESS;
    if (t.has_started && touch_inside_scrollable_area(scrollbar, t)) {
        scrollbar.touch_drag_state = touch_drag.TOUCH_DRAG_PENDING;
        scrollbar.position_on_touch = scrollbar.scroll_position;
    }
    if (t.has_moved && scrollbar.touch_drag_state != touch_drag.TOUCH_DRAG_NONE) {
        scrollbar.touch_drag_state = touch_drag.TOUCH_DRAG_IN_PROGRESS;
...
export function scrollbar_handle_mouse(scrollbar: scrollbar_type, m: mouse) {
    ...
    if (scrollbar.touch_drag_state != touch_drag.TOUCH_DRAG_IN_PROGRESS) {
        if (m.scrolled == scroll_state.SCROLL_DOWN) {
            text_scroll(1, 3);
        } else if (m.scrolled == scroll_state.SCROLL_UP) {
            text_scroll(0, 3);
        }
        if (image_buttons_handle_mouse(m,
            scrollbar.x, scrollbar.y, [image_button_scroll_up], 1, null)) {
            return 1;
        }
        if (image_buttons_handle_mouse(m,
            scrollbar.x, scrollbar.y + scrollbar.height - SCROLL_BUTTON_HEIGHT,
            [image_button_scroll_down], 1, null)) {
            return 1;
        }
    }
