...
import { smacker } from 'core/smacker';
...
import { smacker_frame_status } from 'core/smacker';
import { smacker_open } from 'core/smacker';
import { smacker_close } from 'core/smacker';
import { smacker_get_frames_info } from 'core/smacker';
import { smacker_get_video_info } from 'core/smacker';
import { smacker_get_audio_info } from 'core/smacker';
import { smacker_first_frame } from 'core/smacker';
import { smacker_next_frame } from 'core/smacker';
import { smacker_get_frame_palette } from 'core/smacker';
import { smacker_get_frame_video } from 'core/smacker';
import { smacker_get_frame_audio_size } from 'core/smacker';
import { smacker_get_frame_audio } from 'core/smacker';
...
import { sound_device_use_custom_music_player } from 'sound/device';
import { sound_device_write_custom_music_data } from 'sound/device';
import { sound_device_use_default_music_player } from 'sound/device';
...
function get_next_frame() {
    ...
    let draw_frame: number = data.video.current_frame == 0;
...
export function video_draw(x_offset: number, y_offset: number) {
    ...
    let pal: number = smacker_get_frame_palette(data.s);
    if (frame && pal) {
...
export function video_draw_fullscreen() {
    ...
    let pal: number = smacker_get_frame_palette(data.s);
    if (frame && pal) {
        let scale_w: number = s_width / (double) data.video.width;
        let scale_h: number = s_height / (double) data.video.height * (data.video.y_scale == SMACKER_Y_SCALE_NONE ? 1 : 2);
        let scale: number = scale_w < scale_h ? scale_w : scale_h;
        let video_width: number = (int)(scale * data.video.width);
        let video_height: number = (int)(scale * data.video.height);
        ...
        for (let y: number = clip.clipped_pixels_top; y < video_height - clip.clipped_pixels_bottom; y++) {
            ...
            let video_y: number = (int)((data.video.y_scale == SMACKER_Y_SCALE_NONE ? y : y / 2) / scale);
            let line: number = frame + (video_y * data.video.width);
