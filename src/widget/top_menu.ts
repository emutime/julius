...
import { set_tooltips, setting_cycle_tooltips, setting_monthly_autosave, setting_toggle_monthly_autosave, setting_toggle_warnings, setting_tooltips, setting_warnings } from 'game/settings';
import { game_state_reset_overlay } from 'game/state';
import { system_exit, system_is_fullscreen_only } from 'game/system';
import { game_time_month, game_time_year } from 'game/time';
import { game_undo_disable } from 'game/undo';
import { COLOR_FONT_RED, COLOR_FONT_YELLOW, color_t, COLOR_WHITE } from 'graphics/color';
import { font_t } from 'graphics/font';
import { image_draw } from 'graphics/image';
import { lang_text_draw, lang_text_draw_colored, lang_text_draw_month_year_max_width } from 'graphics/lang_text';
import { menu_bar_draw, menu_bar_handle_mouse, menu_bar_item, menu_draw, menu_handle_mouse, menu_item, menu_update_text } from 'graphics/menu';
import { screen_width } from 'graphics/screen';
import { text_draw_number, text_draw_number_colored } from 'graphics/text';
import { tooltip_context } from 'graphics/tooltip';
import { window_go_back, window_id, window_request_refresh, window_show, window_type } from 'graphics/window';
import { hotkeys } from 'input/hotkey';
import { mouse } from 'input/mouse';
import { scenario_is_custom, scenario_name, scenario_save_campaign_player_name } from 'scenario/property';
import { widget_city_has_input } from 'widget/city';
import { window_advisors_show_advisor } from 'window/advisors';
import { window_city_draw, window_city_draw_all, window_city_draw_panels, window_city_return, window_city_show } from 'window/city';
...
import { popup_dialog_type, window_popup_dialog_show, window_popup_dialog_show_confirmation } from 'window/popup_dialog';
...
import INFO_NONE = info.INFO_NONE;
import INFO_FUNDS = info.INFO_FUNDS;
import INFO_POPULATION = info.INFO_POPULATION;
import INFO_DATE = info.INFO_DATE;

let menu_file: menu_item[] = [
    new menu_item(1, 1, menu_file_new_game, 0),
    new menu_item(1, 2, menu_file_replay_map, 0),
    new menu_item(1, 3, menu_file_load_game, 0),
    new menu_item(1, 4, menu_file_save_game, 0),
    new menu_item(1, 6, menu_file_delete_game, 0),
    new menu_item(1, 5, menu_file_exit_game, 0),
];
let menu_options: menu_item[] = [
    ...
    new menu_item(2, 6, menu_options_difficulty, 0),
    new menu_item(19, 51, menu_options_autosave, 0),
];
let menu_help: menu_item[] = [
    new menu_item(3, 1, menu_help_help, 0),
    new menu_item(3, 2, menu_help_mouse_help, 0),
    new menu_item(3, 5, menu_help_warnings, 0),
    new menu_item(3, 7, menu_help_about, 0),
];
let menu_advisors: menu_item[] = [
    new menu_item(4, 1, menu_advisors_go_to, 1),
    new menu_item(4, 2, menu_advisors_go_to, 2),
    new menu_item(4, 3, menu_advisors_go_to, 3),
    new menu_item(4, 4, menu_advisors_go_to, 4),
    new menu_item(4, 5, menu_advisors_go_to, 5),
    new menu_item(4, 6, menu_advisors_go_to, 6),
    new menu_item(4, 7, menu_advisors_go_to, 7),
    new menu_item(4, 8, menu_advisors_go_to, 8),
    new menu_item(4, 9, menu_advisors_go_to, 9),
    new menu_item(4, 10, menu_advisors_go_to, 10),
    new menu_item(4, 11, menu_advisors_go_to, 11),
    new menu_item(4, 12, menu_advisors_go_to, 12),
];
let menu: menu_bar_item[] = [
    new menu_bar_item(1, menu_file, 6),
    new menu_bar_item(2, menu_options, 5),
    new menu_bar_item(3, menu_help, 4),
    new menu_bar_item(4, menu_advisors, 12),
];
