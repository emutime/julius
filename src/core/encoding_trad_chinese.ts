...
export class chinese_entry {
    public internal: number = 0;
    public utf8: number[] = new Array(3).fill(0);
    public constructor(...args: any[]) {
        args.length >= 1 && (this.internal = args[0]);
        args.length >= 2 && (this.utf8 = args[1]);
    }
}
let codepage_to_utf8: chinese_entry[] = new Array(IMAGE_FONT_MULTIBYTE_TRAD_CHINESE_MAX_CHARS).fill({
...
let utf8_to_codepage: chinese_entry;
export class image_entry {
    public image_id: number = 0;
    public codepage: number = 0;
    public constructor(...args: any[]) {
        args.length >= 1 && (this.image_id = args[0]);
        args.length >= 2 && (this.codepage = args[1]);
    }
}
let image_entries: image_entry[] = new Array().fill({
    { 4, 0xb943},
    { 5, 0xc0b8},
    { 14, 0xc2f7},
    { 15, 0xb67d},
...
function compare_utf8(a: void, b: void) {
    let va: number = ((const chinese_entry*) a).utf8;
    let vb: number = ((const chinese_entry*) b).utf8;
...
function compare_internal(a: void, b: void) {
    let va: number = ((const chinese_entry*) a).internal;
    let vb: number = ((const chinese_entry*) b).internal;
    return va == vb ? 0 : (va < vb ? -1 : 1);
}
export function encoding_trad_chinese_init() {
    if (!utf8_to_codepage) {
        utf8_to_codepage =
...
export function encoding_trad_chinese_to_utf8(input: number, output: char, output_length: number) {
...
export function encoding_trad_chinese_from_utf8(input: char, output: number, output_length: number) {
    if (!utf8_to_codepage) {
        output[0] = 0;
        return;
    }
...
export function encoding_trad_chinese_big5_to_image_id(big5: number) {
    for (let i: number = 0; image_entries[i].image_id; i++) {
