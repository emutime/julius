export const HALF_WIDTH_START = 0;
export const FULL_WIDTH_START = 63;
;
import { language_type } from 'core/locale';
import { encoding_type } from 'core/encoding';
import { encoding_get_utf8_character_bytes } from 'core/encoding';
...
export class japanese_entry {
    public cp932: number = 0;
    public utf8: number[] = new Array(3).fill(0);
    public constructor(...args: any[]) {
        args.length >= 1 && (this.cp932 = args[0]);
        args.length >= 2 && (this.utf8 = args[1]);
    }
}
export class japanese_image_entry {
    public cp932: number = 0;
...
let codepage_to_utf8: japanese_entry[] = new Array().fill({
...
let utf8_to_codepage: japanese_entry;
let num_entries: number = sizeof(codepage_to_utf8) / sizeof(japanese_entry);
let image_id_block_indexes: number[] = new Array(128).fill({
    // 80-8f
    - 1, -1, 0, 1, -1, -1, -1, -1, 1, 2, 3, 4, 5, 6, 7, 8,
    // 90-97
    9, 10, 11, 12, 13, 14, 15, 16, 17
});
let image_id_punctuation_81: number[] = new Array(96).fill({
...
let image_entries: japanese_image_entry[] = new Array().fill({
    { 0x875a, 93},
    { 0x99e1, 94},
    { 0x9aba, 121},
    { 0x9ad0, 122},
...
function compare_utf8(a: void, b: void) {
    let va: number = ((const japanese_entry*) a).utf8;
    let vb: number = ((const japanese_entry*) b).utf8;
...
function compare_codepage(a: void, b: void) {
    let va: number = ((const japanese_entry*) a).cp932;
    let vb: number = ((const japanese_entry*) b).cp932;
    return va == vb ? 0 : (va < vb ? -1 : 1);
}
export function encoding_japanese_init() {
    if (!utf8_to_codepage) {
...
export function encoding_japanese_to_utf8(input: number, output: char, output_length: number) {
...
export function encoding_japanese_from_utf8(input: char, output: number, output_length: number) {
    if (!utf8_to_codepage) {
        output[0] = 0;
        return;
    }
...
function compare_image(a: void, b: void) {
    let va: number = ((const japanese_image_entry*) a).cp932;
    let vb: number = ((const japanese_image_entry*) b).cp932;
    return va == vb ? 0 : (va < vb ? -1 : 1);
}
export function encoding_japanese_sjis_to_image_id(first: number, second: number) {
    if (first < 0x80) {
        return -1;
        ...
        let block_index: number = image_id_block_indexes[first - 0x80];
        let char_index: number = second - 0x40 - (second >= 0x80 ? 1 : 0);
        return FULL_WIDTH_START + block_index * 188 + char_index;
    } else {
        let key: japanese_image_entry = {(first << 8) | second
    };
    let entry: japanese_image_entry = bsearch(key, image_entries,
