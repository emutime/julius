...
export class korean_entry {
    public cp949: number = 0;
    public utf8: number[] = new Array(3).fill(0);
    public constructor(...args: any[]) {
        args.length >= 1 && (this.cp949 = args[0]);
        args.length >= 2 && (this.utf8 = args[1]);
    }
}
let codepage_to_utf8: korean_entry[] = new Array(IMAGE_FONT_MULTIBYTE_KOREAN_MAX_CHARS).fill({
...
let utf8_to_codepage: korean_entry;
function compare_utf8(a: void, b: void) {
    let va: number = ((const korean_entry*) a).utf8;
    let vb: number = ((const korean_entry*) b).utf8;
...
function compare_codepage(a: void, b: void) {
    let va: number = ((const korean_entry*) a).cp949;
    let vb: number = ((const korean_entry*) b).cp949;
    return va == vb ? 0 : (va < vb ? -1 : 1);
}
export function encoding_korean_init() {
    if (!utf8_to_codepage) {
...
export function encoding_korean_to_utf8(input: number, output: char, output_length: number) {
...
export function encoding_korean_from_utf8(input: char, output: number, output_length: number) {
    if (!utf8_to_codepage) {
        output[0] = 0;
        return;
    }
